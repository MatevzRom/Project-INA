...
ax.set_title(f"PEMS08 — daily congestion pattern (positive rate = {positive_rate:.3f})")
ax.grid(alpha=0.3)
out = REPORTS / "pems08_daily_congestion.png"
fig.tight_layout()
...
ax.set_title("PEMS08 — degree distribution")
out = REPORTS / "pems08_degree_distribution.png"
fig.tight_layout()
fig.savefig(out, dpi=150)
plt.close(fig)
print(f"\nsaved → {out}")
...
ax.set_title("PEMS08 sensor network — spring layout")
ax.axis("off")
out = REPORTS / "pems08_network_layout.png"
fig.tight_layout()
fig.savefig(out, dpi=150)
plt.close(fig)
print(f"saved → {out}")
