...
\begin{frame}{Traffic speed forecasting is a graph time-series problem}
\footnotesize
\begin{columns}[T,totalwidth=\textwidth]
\begin{column}{0.48\textwidth}
\textbf{Question}

Given recent measurements from freeway sensors, how fast will traffic move later at each sensor?

\vspace{0.6em}
\textbf{This project}

We compare neural models for two forecasting ranges:
\begin{itemize}
  \item near future: +5 to +60 minutes
...
PEMS08 is a node-level regression task on a road graph: each sensor has its own time series, and neighbouring sensors provide traffic context.

\vspace{0.6em}
Speed forecasts support travel-time estimation, congestion warnings, and routing decisions.
\end{column}
\begin{column}{0.47\textwidth}
\begin{figure}
  \centering
  \includegraphics[width=\linewidth,height=0.54\textheight,keepaspectratio]{pems08_daily_congestion.png}
\end{figure}
\vspace{-0.7em}
\centering
\footnotesize Congestion follows daily structure, but is rare overall: 2.27\% of sensor-time points.
\end{column}
\end{columns}
\end{frame}
...
\begin{frame}{PEMS08 gives us a sparse but connected road graph}
\footnotesize
\begin{columns}[T,totalwidth=\textwidth]
\begin{column}{0.55\textwidth}
\begin{figure}
  \centering
  \includegraphics[width=\linewidth,height=0.60\textheight,keepaspectratio]{pems08_network_layout.png}
\end{figure}
\end{column}
...
\begin{itemize}
  ...
  \item one weakly connected component
  \item average in/out-degree 1.63
...
Each sensor has its own time series, but nearby sensors provide useful context.
\end{column}
\end{columns}
\end{frame}
...
\begin{frame}{NN3 predicts later-today traffic speed}
\footnotesize
Near-future forecasting is useful for immediate control. We also predict farther ahead, where daily traffic rhythm becomes important.
...
\begin{itemize}
  \item one speed prediction per sensor
  \item four horizons predicted together
\end{itemize}
\end{column}
\begin{column}{0.47\textwidth}
\textbf{Leakage-safe inputs}
\begin{itemize}
  \item current flow, occupancy, speed at time $t$
  \item speed lags: 3h, 6h, 12h, 1d, 2d, 7d
  \item current and target time-of-day/day-of-week
\end{itemize}
...
\vspace{0.35em}
\centering
No future traffic measurement is used as an input feature.
\end{frame}

\begin{frame}{NN3 uses one shared GRU for all sensors}
\small
\begin{center}
\large
...
\begin{itemize}
  \item keeps the model small and stable
  \item learns temporal patterns shared by all sensors
  \item predicts all long horizons in one pass
\end{itemize}
\end{column}
\begin{column}{0.47\textwidth}
\textbf{Best run}
...
\vspace{0.6em}
\centering
Train-only normalisation is used, and metrics are reported after denormalising back to mph.
\end{frame}
...
\missingfigure{0.55\textheight}{Per-horizon MAE crossover plot\\Persistence vs. NN2\\Use teammate figure when available.}
\end{column}
...
\small
\missingfigure{0.48\textheight}{Ablation figure\\Fixed graph vs. adaptive adjacency vs. residual\\Use teammate figure when available.}
...
\begin{frame}{NN3 result: lower error at every long horizon}
\footnotesize
\begin{columns}[T,totalwidth=\textwidth]
...
The hardest point is +6h; +12h becomes easier because daily periodicity helps.
\end{column}
\end{columns}
\end{frame}
...
\large
\begin{itemize}
  \item PEMS08 is naturally a graph time-series forecasting problem.
  \item The short-horizon models predict immediate traffic dynamics.
  \item NN3 extends the project to later-today forecasting: +1h, +3h, +6h, +12h.
  \item Chronological splits and leakage checks make the reported scores realistic.
  \item The best long-horizon GRU beats the strongest baseline at every horizon.
\end{itemize}
\end{frame}
